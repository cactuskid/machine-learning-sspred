...
def start_workers(client,tf_spec, dask_spec , keras_model , verbose  ):
	worker_tasks = [client.submit(worker_task, tf_spec = tf_spec,dask_spec = dask_spec, verbose= verbose , keras_model = keras_model, workers=worker , pure=False) for worker in dask_spec['worker']]
...
def scoring_task(tf_spec , xval, yval, keras_model , verbose = False):
	...
	with local_client() as c:
		...
		with sess:
			while True:
				score = sess.eval( loss, feed_dict= {x: X_test,
												labels: y_test ,
												K.learning_phase(): 0})
				scores.append(float(score))
				if verbose== True:
					print(score) 
				if time.clock() - start> 3600:
					#EVALUATE THE MODE AND SAVE TO LOG
...
def worker_task(tf_spec, dask_spec , verbose , keras_model):                                                           
	...
	while not scores or scores.data[-1] > 1000:
		try:
			...
			continue
		train_data = {x: batch[0], y_: batch[1] ,  K.learning_phase(): 1 }
...
def model(server, tf_spec , dask_spec , modelfun , sync_replicas = False , chkptdir='./' ):
	...
	with tf.device(tf.train.replica_device_setter(worker_device=worker_device, cluster=tf_spec)):
		...
		return sess, x, y_, train_step, global_step, loss , encode_y , encode_labels , confusion
...
def configure_cluster( client, dask_cluster , tf_spec, dask_spec  , retmodel , verbose=True):
