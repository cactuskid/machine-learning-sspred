...
with tf.Session(config=tf.ConfigProto(
                    intra_op_parallelism_threads=30)) as sess:
    K.set_session(sess)
...
print(fastas)import tensorflow as tf
from keras.backend import tensorflow_backend as K
...
for n in range(LSTMlayers):
	if n == 0:
		if conv2dlstm == False:
			...
			layer = ConvLSTM2D(LSTMoutdim, kernel_size, name ='lstm_'+str(n), strides=(1, 1), padding='valid', data_format=None, dilation_rate=(1, 1), activation='relu', recurrent_activation='hard_sigmoid', use_bias=True, kernel_initializer='glorot_uniform', recurrent_initializer='orthogonal',
	...
	for j in range(testsamples):
		...
		for j in range(TRAIN_epochs):
			for i in range(X.shape[0]):
				...
				performance = model.test_on_batch( [np.expand_dims(X[i,:,:], axis=0),np.expand_dims( FFTX, axis =0 ) , position , position],  [ Y[i,:], Y[i,:]] )
				accuracy_rnn.append(performance[3])
				accuracy_deep.append(performance[4])
				loss_rnn.append(performance[1])
				loss_deep.append(performance[2])
		...
		test_accuracy_rnn.append(ar)
		test_accuracy_deep.append(ad)
		test_loss_rnn.append(lr)
		test_loss_deep.append(dl)
